...
write.csv(table2b, paste0(diroutput,"sccs_sensitivity/", "/table_2b_event_dependency_by_type.csv"), row.names = FALSE)
...
for (i in dose1_names) {
  ...
  plot <- dose1_plots[[i]]
  file_name = paste0(diroutput,"sccs_sensitivity/","plot_d1_event_dependency_plot_", i, ".pdf")
...
for (i in dose2_names) {
  ...
  plot <- dose2_plots[[i]]
  file_name = paste0(diroutput,"sccs_sensitivity/","plot_d2_event_dependency_plot_", i, ".pdf")
  ggsave(file_name, plot = plot)
  while (!is.null(dev.list()))  dev.off()
...
for (i in dose3_names) {
  ...
  plot <- dose3_plots[[i]]
  file_name = paste0(diroutput,"sccs_sensitivity/","plot_d3_event_dependency_plot_", i, ".pdf")
  ggsave(file_name, plot = plot)
  while (!is.null(dev.list()))  dev.off()
